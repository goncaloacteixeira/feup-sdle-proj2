...
const cleanEphemeral = function (node) {
  RECORDS.forEach((value, key, map) => {
    ...
    if (value.record.posts.length > EPHEMERAL_MAX_POSTS) {
      ...
      value.record.posts = value.record.posts.slice(
        value.record.posts.length - EPHEMERAL_MAX_POSTS
      );
    }
...
const get_not_followers = async function (node, username) {
  return new Promise(async (resolve) => {
    ...
    node.dialProtocol(peerId, ["/followers/1.0.0"]).then(
      async ({ stream }) => {
...
const retrieve_followers = async function (node) {
  ...
  if (node.pubsub.started && toRemove.length > 0)
    exports
      .put_record(node, node.application)
...
const trigger_update_record = async function (node, username) {
  ...
  node.dialProtocol(peerId, ["/update/1.0.0"]).then(
    async ({ stream }) => {
...
const update_records = function (node) {
  node.application.subscribed.forEach((username) =>
    trigger_update_record(node, username)
  );
};
...
exports.create_node = async function create_node(username, peerIdJSON) {
    ...
    if (BOOTSTRAP_IDS.includes(connection.remotePeer.toB58String())) {
      ...
      if (providers.length === 0) {
        ...
        while (!done && i < providers.length) {
          ...
          try {
            dial = await node.dialProtocol(providers[i++].id, [
              "/record/1.0.0",
            ]);
          } catch (e) {
            continue;
  ...
  node.handle("/followers/1.0.0", async ({ stream }) => {
    ...
    pipe(
      [node.application.subscribed.includes(username) ? "YES" : "NO"],
      stream
    );
...
exports.put_record = async function (node, record) {
  node.application = record;
  set_record(
    (await username_cid(node.application.username)).toString(),
    record
  );
