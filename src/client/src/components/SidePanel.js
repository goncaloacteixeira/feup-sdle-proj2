...
import {Grid, CircularProgress, Typography, TextField, InputAdornment} from "@mui/material";
...
export default function SidePanel() {
  ...
  React.useEffect(() => {
    axios.get("/users").then((res) => {
      setUsernames(removeSelf(res.data));
    });
  }, [info]);
  ...
  const removeSelf = (users) => {
    const index = users.indexOf(localStorage.getItem('username'));
    if (index > -1) {
